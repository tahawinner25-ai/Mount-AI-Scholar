...
export default function PhonemeOrbit({ user, onBack, selectedLang }: PhonemeOrbitProps) {
  ...
  const [activeAccentId, setActiveAccentId] = useState<string | null>(null);
...
Return ONLY a valid JSON object matching this structure (do not wrap in markdown code blocks):
{
  ...
  const handleSpeakWord = async (textToSpeak: string, useSelectedAccent = true) => {
    ...
    const activePreset = PRESET_WORDS[selectedPresetIdx];
    try {
      const accentToUse = useSelectedAccent && activeAccentId ? activeAccentId : undefined;
      ...
      setErrorLogs(prev => [...prev, `🔌 Échec connexion API TTS. Inférence vocale locale engagée.`]);
    }
  ...
  const handleSpeakAccent = async (textToSpeak: string, accentId: string) => {
    ...
    try {
      ...
      setErrorLogs(prev => [...prev, `🔌 Échec connexion accent API. Inférence vocale locale.`]);
    }
  ...
  const handleSpeakPhoneme = async (char: string, ipa: string) => {
    if (isSpeakingTts) return;
    setIsSpeakingTts(true);
    const activePreset = PRESET_WORDS[selectedPresetIdx] || PRESET_WORDS[0];
    try {
      const promptText = activePreset.lang === "French"
        ? `Prononcez le phonème ou son individuel "${char}" (représenté phonétiquement par [${ipa}]) de manière très claire, isolée et prolongée, comme un instituteur apprenant la phonétique à un enfant.`
        : `Pronounce the individual phoneme or sound "${char}" (phonetically represented as [${ipa}]) very clearly, in isolation, prolonged, like a speech language pathologist teaching phonics.`;
      ...
      console.error("TTS phoneme error:", err);
    }
  ...
  const handleSelectAccent = (accentId: string) => {
    if (activeAccentId === accentId) {
      setActiveAccentId(null);
    } else {
      setActiveAccentId(accentId);
      handleSpeakAccent(activePreset.word, accentId);
    }
  };
        ...
        <div className="lg:col-span-7 bg-[#0b0f19]/80 backdrop-blur-xl rounded-[2.5rem] border border-slate-800/80 p-8 flex flex-col justify-between relative overflow-hidden min-h-[500px] shadow-2xl">
          ...
          <div className="flex-1 flex items-center justify-center relative min-h-[350px]">
            ...
            >
              ...
              <div className="absolute -inset-4 rounded-full border border-indigo-500/10 group-hover:bg-indigo-500/5 transition-colors animate-pulse" />
              
              {/* Core Capsule with Glassmorphism */}
              <div className="w-full h-full rounded-full bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-900 border border-indigo-500/50 shadow-[0_0_50px_rgba(99,102,241,0.4)] flex flex-col items-center justify-center p-4 group-hover:border-indigo-400 group-hover:shadow-[0_0_60px_rgba(99,102,241,0.6)] transition-all duration-300">
                <span className="text-[9px] font-mono text-indigo-400 font-black tracking-widest uppercase mb-1 flex items-center gap-1">
                  <Volume2 className={`w-3 h-3 text-indigo-400 ${isSpeakingTts ? 'animate-bounce' : 'animate-pulse'}`} /> 
                  {isSpeakingTts ? 'SPEAKING' : 'GRAVITY CORE'}
                </span>
                <p className="text-xl font-black text-white tracking-tighter uppercase text-center group-hover:text-indigo-300 transition-colors">{activePreset.word}</p>
                <span className="text-[10px] text-indigo-300 font-mono font-medium mt-1">{activePreset.ipa}</span>
                
                {activeAccentId ? (
                  <div className="mt-1.5 flex items-center gap-1 bg-indigo-500/15 border border-indigo-500/30 px-1.5 py-0.5 rounded-full text-[8px] font-mono text-indigo-300">
                    <span>{ACCENTS.find(a => a.id === activeAccentId)?.flag}</span>
                    <span className="uppercase font-bold text-[7px]">{ACCENTS.find(a => a.id === activeAccentId)?.name}</span>
                  </div>
                ) : (
                  <span className="text-[8px] text-slate-500 font-mono uppercase tracking-wide mt-2 opacity-0 group-hover:opacity-100 transition-all duration-200">Click to Speak</span>
                )}
              </div>
            </button>
          ...
          <div className="border-t border-slate-900/80 pt-6 mt-4 relative z-10 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              {ACCENTS.map((acc) => {
                const isSelected = activeAccentId === acc.id;
                const isSpeakingThis = activeAccentSpeaking === acc.id;
                return (
                  <button
                    key={acc.id}
                    onClick={() => handleSelectAccent(acc.id)}
                    disabled={isSpeakingTts && !isSpeakingThis}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-left transition-all border cursor-pointer ${
                      isSelected
                        ? 'bg-indigo-500/20 border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.3)] text-indigo-300'
                        : isSpeakingThis
